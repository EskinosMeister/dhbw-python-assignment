# reset_db.py
"""
Label: Datenbank-Reset und Testdaten-Seeding
...
Kurzbeschreibung des Moduls:
    Dieses Skript dient dazu, die SQLite-Datenbank (`grocery.db`) komplett zurückzusetzen, 
    neu aufzubauen und mit umfassenden Testdaten zu befüllen. 
    Es ist primär für Entwicklungs- und Testzwecke gedacht.
...
def main():
    """
   Label: Datenbank zurücksetzen und befüllen
    Kurzbeschreibung:
        Die Hauptfunktion des Skripts. Sie löscht die bestehende Datenbankdatei, 
        erstellt das Schema neu aus `schema_sql` und befüllt alle Tabellen mit vordefinierten Testdaten 
        (User, Supermärkte, Produkte, Preise und Beispiel-Bestellungen).
    ...
    Tests:
        1. Reset: Die Datei `grocery.db` wird gelöscht und erfolgreich neu erstellt.
        2. Schema: Alle 7 Tabellen (`users` bis `saved_products`) existieren und haben die korrekte Struktur.
        3. Daten: Es werden mindestens 1 User, 3 Supermärkte, 4 Produkte und 7 Bestellungen (Orders) eingefügt.
    ...
    def d(days_back: int) -> str:
        """ Kleine Hilfsfunktion für Order-Datum"""
